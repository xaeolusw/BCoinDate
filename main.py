...
def get_nei_time(distance,speed):
    return (distance * 10000)/speed/60
...
neit_time = get_nei_time(distance,speed)
print(f"地址{begin_addr}到{target_addr}的距离是{distance}吉米，速度是{speed}米/分钟，需要{neit_time}分钟。")
#将一个表示分钟的浮点数转为时间格式，如138.5分钟转为2小时18分钟.公式为：小时=整数(分钟/60),分钟=余数(分钟/60)。编程语言中的整数函数为int()，余数函数为%。即
#print(f"{int(neit_time/60)}小时{int(neit_time%60)}分钟")
#设定一个到达时间，一个需要时间，从而计算出发时间。如到达时间为12:00，需要时间为2小时18分钟，则出发时间为9:42。具体计算公式为：出发时间=到达时间-需要时间。编程语言中的时间函数为time()，时间格式化函数为strftime()。
print(t.strftime("%H:%M", t.localtime(t.mktime(t.strptime("2013-05-19 12:00","%Y-%m-%d %H:%M")) - neit_time * 60)))
