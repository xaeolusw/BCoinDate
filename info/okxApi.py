...
询价方使用POST /api/v5/rfq/create-rfq创建询价单。询价方可以可通过GET /api/v5/public/instruments查询可询价产品信息，即通过GET /api/v5/rfq/counterparties查询可选择报价方信息。
询价方可以在询价单有效时任何时候通过POST /api/v5/rfq/cancel-rfq取消询价单。
...
对于同一杠杆产品，所有反方向挂单的币数加上当前只减仓下单数量，不能超过仓位资产
...
当委托价格对0.0005的余数小于等于0.00025，且委托价格大于0.0005时，向下取。
 下单附带止盈止损，只有当该订单完全成交时，才会生成止盈止损策略订单，否则不会生成止盈止损策略订单。
 自成交保护
...
fillTime	String	成交时间，与订单频道的fillTime相同
 tradeId
当成交明细所归属的订单种类（category）为 partial_liquidation：强制减仓、full_liquidation：强制平仓、adl：ADL自动减仓时，tradeId字段的值为"0"
tag
...
effective： 已生效
canceled：已撤销
order_failed：委托失败 partially_failed：部分委托失败
lever	String	杠杆倍数，0.01到125之间的数值，仅适用于 币币杠杆/交割/永续
tpTriggerPx	String	止盈触发价
tpTriggerPxType	String	止盈触发价类型
last：最新价格
...
全仓下，instId和ccy至少要传一个；如果两个都传，默认使用instId
ccy	String	可选	保证金币种
仅适用于跨币种保证金模式和组合保证金模式的全仓 币币杠杆。设置自动借币的杠杆倍数时必填
lever	String	是	杠杆倍数
mgnMode	String	是	保证金模式
isolated：逐仓 cross：全仓
...
quick_margin:一键借币
 衍生品
开仓划转：在开仓和平仓时自动占用和释放保证金 自主划转：需要手动给仓位划入保证金然后开始交易。该选项主要供专业用户使用，会有初次划入仓位资金≥10,000USDT的限制。
 杠杆
开仓划转：在开仓和平仓时自动借币和还币 自主划转：需要手动给仓位划入保证金然后交易，同时也支持手动转出借币。该选项主要供专业用户使用，会有初次划入仓位资金≥10,000USDT的限制。
查看账户最大可转余额
...
适用于交割/永续/期权
uly与instFamily必须传一个,若传两个，以instFamily为主
...
1：子账户转母账户
subAcct	String	否	子账户名称
after	String	否	查询在此之前的内容，值为时间戳，Unix时间戳的毫秒数格式，如 1597026383085
before	String	否	查询在此之后的内容，值为时间戳，Unix时间戳的毫秒数格式，如 1597026383085
limit	String	否	分页返回的结果集数量，最大为100，不填默认返回100条
返回结果
...
请求参数
参数名	类型	是否必须	描述
enable	Boolean	是	为子账户设置 VIP 尊享借币可用额度的比率。
true 或者 false
alloc	Array of objects	否	如果 enable = false, 将不会验证该参数
> subAcct	String	是	子账户名称
> loanAlloc	String	是	子账户尊享借币可用额度的比率（百分比）
范围为[0, 100]. 精度为 0.01% (2位小数)
"0" 代表取消子账户的 VIP 借币额度。
返回结果
...
> rate	String	日利率
> loanQuota	String	母账户维度借币限额
如果 loanAlloc 不为 ""，该字段代表当前交易账户（子账户）的借币限额
> surplusLmt	String	母子账户剩余可借
如果 loanAlloc 不为 ""，该字段代表当前交易账户（子账户）的剩余可借
> usedLmt	String	母子账户已借额度
如果 loanAlloc 不为 ""，该字段代表当前交易账户（子账户）的已借额度
> interest	String	可忽略，只返回 “”
> posLoan	String	当前账户负债占用（子账户，锁定额度内）
仅适用于尊享借币
> availLoan	String	当前账户剩余可用（子账户，锁定额度内）
仅适用于尊享借币
> usedLoan	String	当前账户（子账户）已借额度
仅适用于尊享借币
> avgRate	String	币种已借平均(小时)利率，仅适用于尊享借币
网格交易
网格是一种在指定价格区间自动进行低买高卖的交易策略。用户设定参数后，系统分割小网格自动挂单，随着市场波动，策略低买高卖赚取波段收益。了解更多
...
tdMode	String	是	交易模式
跨币种/PM保证金模式下选择 全仓：cross
简单/单币种模式下选择 非保证金：cash
...
GET /api/v5/tradingBot/recurring/orders-algo-details?algoId=560473220642766848
请求参数
参数名	类型	是否必须	描述
algoId	String	是	策略订单ID
返回结果
...
赚币
赚币功能模块下的API接口需要身份验证。 仅资金账户中的资产支持赚币。
...
申请流程可以参考 如何申请成为交易员?。不支持跟单员使用跟单API；
可通过查看账户配置接口的roleType是否为 1，判断当前账户是否为带单交易员。
2. 带单合约

交易员获取带单合约接口，用于查看平台哪些合约支持带单，以及您开启了哪些合约的带单。对于您未开启带单的合约，依旧可以正常交易，只是不会触发跟单；
交易员修改带单合约接口，初始带单合约在申请带单交易员时进行设置，该接口用于修改您的带单合约。非带单合约修改为带单合约时，该次请求中所有的非带单合约合约不能有持仓或者挂单。
3. 开仓

需要通过下单接口和频道进行开仓，包括：下单接口、批量下单接口、下单频道、批量下单频道，
在买卖模式下，委托的方向必须与现有持仓和挂单保持一致，如果对应产品没有持仓和挂单，可根据自己的需求选择委托方向；
开平仓模式下，可根据自己的需求选择开多或开空。
4. 平仓

不支持通过下单接口和频道自定义数量进行平仓，需要通过市价仓位全平接口或者交易员平仓接口进行平仓；
市价仓位全平接口，平掉当前产品下指定的仓位（如：开平模式下，全仓模式下的多仓或空仓），可能包含多个跟单
交易员平仓接口，一次仅平仓某一个带单仓位。带单ID（subPosId）为必填参数，需要通过交易员获取当前带单接口获取。
交易员获取当前带单
交易员获取当前未平仓的带单仓位。
...
交易员获取历史带单
交易员获取最近三个月的已经平仓的带单仓位，按照平仓时间倒序排序。
...
交易员平仓
交易员一次仅可平仓一个带单仓位。
带单ID（subPosId）为必填参数，需要通过交易员获取当前带单接口获取。
...
交易员修改带单合约
交易员修改带单合约的设置。初始带单合约在申请带单交易员时进行设置。
非带单合约修改为带单合约时，该次请求中所有的非带单合约合约不能有持仓或者挂单。
...
expTime	String	交割/行权日期，仅适用于交割 和 期权
Unix时间戳的毫秒数格式，如 1597026383085
...
默认返回全部类型
uly	String	可选	标的指数
交割/永续/期权情况下，uly与instFamily必须传一个，若传两个，以instFamily为主
instFamily	String	可选	交易品种
交割/永续/期权情况下，uly与instFamily必须传一个，若传两个，以instFamily为主
...
serviceType	String	服务类型， 0：WebSocket ; 5：交易服务；6：大宗交易；7：策略交易；8：交易服务 (按账户分批次)；9：交易服务 (按产品分批次)；99：其他（如：停止部分产品交易）
...
optimal_limit_ioc：市价委托立即成交并取消剩余（仅适用交割、永续）
> sz	String	是	委托数量
> px	String	否	委托价，仅适用于limit、post_only、fok、ioc类型的订单
> reduceOnly	Boolean	否	是否只减仓，true 或 false，默认false
仅适用于币币杠杆，以及买卖模式下的交割/永续
仅适用于单币种保证金模式和跨币种保证金模式
...
首次推送，只推用户币种层面资产不为0的账户信息。币种层面资产不为0的定义：eq、availEq、availBal 中任意一个字段不为0，即币种层面资产不为0。如果数据太大无法在单个推送消息中发送，它将被分成多个消息发送。
...
首次推送，只推用户持有的仓位。用户持仓仓位定义：持逐仓自主划转模式下的逐仓仓位pos=0，pos>0或者pos<0都认为持有仓位。如果数据太大无法在单个推送消息中发送，它将被分成多个消息发送。
...
> ccy	String	币种
> ts	String	获取greeks的时间，Unix时间戳的毫秒数格式，如 1597026383085
首次推送，只推账户资产不为0的greeks数据。如果数据太大无法在单个推送消息中发送，它将被分成多个消息发送。
...

{
    ...
    "data": [{
        ...
        "minPx": "400",
        "pTime": "1682501933695",
        "perMaxProfitRate": "0.2847923478977315",
        "perMinProfitRate": "0.2847923478977315",
        "pnlRatio": "0",
        "profit": "0",
        "quoteSz": "25",
        "rebateTrans": [{
...

{
        ...
        "data": [{
                ...
                "cycles": "1",
                "instType": "SPOT",
                "investmentAmt": "99.998915016",
                "investmentCcy": "USDC",
                "mktCap": "97.8967926",
                "pTime": "1679984647016",
                "period": "daily",
                "pnlRatio": "-0.0210214522393934",
                "recurringDay": "1",
                "recurringList": [{
                        "avgPx": "27961.6",
                        "ccy": "BTC",
                ...
                "tag": "",
                "timeZone": "8",
                "totalAnnRate": "-7.672830067378591",
                "totalPnl": "-2.102122416",
                "uTime": "1679954410854"
        }]
}
推送数据参数
...
> lever	String	该instId支持的最大杠杆倍数，不适用于币币、期权。可用来分辨币币杠杆和币币
> tickSz	String	下单价格精度，如 0.0001
...
异常情况：
1. 如果一段时间内没有深度更新，OKX将发一条空的消息("date":[])以通知用户连接是正常的。推送的seqId跟上一条信息的一样，prevSeqId等于seqId。 2. 序列号可能由于维护而重置，在这种情况下，用户将收到一条seqId小于prevSeqId的增量消息。随后的消息将遵循常规的排序规则。
...
50052	200	由于您所在国家或地区的合规限制，您无法交易此币对
API 类
错误码	HTTP 状态码	错误提示
...
51008	200	委托失败，因为 {param0} 剩余的档位限额不足，导致可借不足（需借：{1}， 剩余额度{2}，限额 {3}，已用额度{4}）
51008	200	委托失败，因为 {param0} 剩余的母账户限额不足，导致可借不足（需借 {1}，剩余额度 {2}，限额 {3}，已用额度 {4}）
51008	200	委托失败，因为 {param0} 剩余的币对限额不足，导致可借不足
51008	200	委托失败，因为 {param0} 剩余的借贷池限额不足，导致可借不足
51008	200	委托失败，账户资产不足，美金层面有效保证金小于 IMR
51008	200	委托失败，delta 校验未通过，因为若成功下单，adjEq 的变化值将小于 IMR 的变化值。建议增加 adjEq 或减少 imr 占用
...
51502	200	修改订单失败，账户 {param0} 可用保证金不足，且未开启自动借币
...
51502	200	修改订单失败，账户资产不足，美元层面有效保证金小于 IMR
51502	200	修改订单失败，Delta 校验未通过，因为若成功下单，adjEq 的变化值将小于 IMR 的变化值。建议增加 adjEq 或减少 IMR 占用。
